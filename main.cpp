	...
	*/
		...
		for (auto & sfm_it : pks_fields){
			...
			if (sfm.field_desc->type() == FieldDescriptor::TYPE_FLOAT ||
				...
				WRITE_LINE("%s.length == rhs.%s.length && memcmp(%s.data, rhs.%s.data, %s.length) == 0 &&",
					sfm.GetVarName().c_str(), sfm.GetVarName().c_str(),
					sfm.GetVarName().c_str(), sfm.GetVarName().c_str(), sfm.GetVarName().c_str());
			}
			else {
				WRITE_LINE("%s == rhs.%s &&", sfm.GetVarName().c_str(), sfm.GetVarName().c_str());
			}
		}
		WRITE_LINE("true;");
		--level;
		...
		for (auto & sfm_it : pks_fields){
			...
			if (sfm.field_desc->type() == FieldDescriptor::TYPE_STRING){
				...
				WRITE_LINE("memcmp(%s.data, rhs.%s.data, std::min(%s.length, rhs.%s.length) < 0 &&",
					sfm.GetVarName().c_str(), sfm.GetVarName().c_str(),
					sfm.GetVarName().c_str(), sfm.GetVarName().c_str());
			}
			else {
